...
import { useEmCompare } from "@/src/hooks/data/useEntitlementManagement";
import { InlineError } from "@/src/components/error/InlineError";
import { presentError } from "@/src/lib/errors/getErrorPresentation";
import type { EmCompareCellStatus, EmCompareGrid } from "@/src/types/emPackages";

const CELL: Record<EmCompareCellStatus, { label: string; variant: BadgeVariant }> = {
...
export default function Page() {
  ...
  const { grid, compare, isLoading, error } = useEmCompare();
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Entitlement Management — Governance Board"
        description="Governance scorecard across the tenants you manage — never-expiring packages, stale guests, and failing workflows per entity. N/A means the tenant isn't licensed for Entra ID Governance and is not assessed (not a clean pass)."
        breadcrumbs={[
          { label: "Entra ID", href: "/dashboard/entra-id", icon: ShieldCheck },
          { label: "Entitlement Management", href: "/dashboard/entra-id/entitlement-management", icon: ShieldCheck },
          { label: "Governance Board" },
        ]}
        action={
          <Button variant="outline" size="sm" leftIcon={<ArrowLeft size={14} />} asChild>
            <Link href="/dashboard/entra-id/entitlement-management">Back</Link>
          </Button>
        }
      />
      ...
      {(isLoading || grid) && <GovernanceBoard grid={grid} loading={isLoading} />}
    </div>
  );
}

function GovernanceBoard({ grid, loading }: { grid: EmCompareGrid | null; loading: boolean }) {
  const columns: DtColumn<EmCompareGrid["tenants"][number]>[] = [
    { key: "name", header: "Tenant", sortable: true, render: (_, tenant) => <span className="text-xs font-medium text-foreground">{tenant.name}</span> },
    {
      ...
      align: "center",
      render: (_, tenant) => (tenant.emLicensed ? <span className="text-xs text-foreground">{tenant.expiringSoon}</span> : <Badge variant="neutral">Requires Governance</Badge>),
    },
    ...(grid?.checks ?? []).map((check): DtColumn<EmCompareGrid["tenants"][number]> => ({
      key: check.id,
      header: check.label,
      align: "center",
      render: (_, tenant) => {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-(--custom-table-border) bg-(--custom-table-bg)">
      <DataTable<EmCompareGrid["tenants"][number]>
        data={grid?.tenants ?? []}
        columns={columns}
        ...
        defaultSortDir="asc"
        emptyState={{ icon: ShieldCheck, title: "No tenants to compare", description: "None of the selected tenants are accessible to you with governance read permission." }}
      />
    </div>
  );
}
