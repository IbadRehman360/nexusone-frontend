...
| `SectionCard` | `src/components/ui/display/SectionCard.tsx` | Titled card with optional subtitle + "View all →" link (dashboard-overview building block — scan history, activity feeds, coverage bars). Extracted from a duplicated local copy in Purview's Overview page; a near-identical hand-rolled version (`rounded-2xl ... bg-card p-5`, using the wrong static `bg-card` token instead of `bg-(--custom-table-bg)`) had also been copy-pasted across ~10 Entra ID premium-feature pages (entitlement-management, conditional-access, identity-protection, access-reviews, PIM) before being fixed — always use this component for a titled overview card, never hand-roll the container again. |
...
- `src/lib/sampleData/{powerPlatform,entraId,purview}.ts` — static fixtures. Wired into Home (all 6 PP stat cards) + Entra Overview + Users, Purview Overview (DLP card/sections only — its other stat cards are NexusOne-owned infra, stay real/unconditional) + Purview DLP. **Power Platform is now fully gated end to end** — every PP page (Overview, Environments + detail, Business Units, Environment Groups, Teams, Users, Security Roles, Resources, Environmental Compliance + detail/history, DLP Policies, Import, Backups, Delegations) shows sample data + locked actions + `ModuleConnectBanner` when not connected, not just the original 2-page scope. This happened because NexusOne's own test tenant is the app registrations' home tenant, so real API calls "just work" even when a module isn't purchased in our system — masking the intended locked experience during testing, which is what prompted expanding coverage. Entra ID and Purview's deeper pages (Groups, Enterprise Apps, Conditional Access, Data Map, Catalog, etc.) are still NOT gated — that expansion wasn't requested, only Power Platform's was.
